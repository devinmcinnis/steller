$(document).ready(function() {
		...
		if ( size >= mq ) {
			...
			for ( var i in markers ) {
				...
				// If this marker is neither the depart or return city, change it's icon.
					...
					markers[i].setIcon('/img/markers/marker_' + markers[i]['title'] + '.png');
				}
	...
	function getQuery(dir) { 
		...
		var formatdptDate = dptdate.getDate() + "-" + (dptdate.getMonth() + 1) + "-" + dptdate.getFullYear(),
			formatrtnDate = rtndate.getDate() + "-" + (rtndate.getMonth() + 1) + "-" + rtndate.getFullYear();
	...
	function addFlights(page, flight, dir) {
		...
		// If 'page4' is implied, append new flights to the 'Confirm Flights' section on page 4.
			...
			if ( $('div.departing-flight').hasClass('selected') ) {
				...
				var taxes = (parseInt(cfdepart.text().substr(1,3), 10) + parseInt(cfreturn.text().substr(1,3),10)) * 0.18;
				var total = (parseInt(cfdepart.text().substr(1,3), 10) + parseInt(cfreturn.text().substr(1,3),10)) + taxes;
				// Send the new taxes and totals to the confirmation page.
				$('ol.confirmed-price li:nth-child(3)').text('$' + taxes);
				$('ol.confirmed-price li:nth-child(4) strong').text('$' + total);
	...
	function logNewFlights(n) {
		for ( var i = 0; i < n; i += 1 ) {
			var num = Math.floor(Math.random() * 1500 + 1),
				cities = ['Nanaimo', 'Sechelt', 'Tofino', 'Vancouver', 'Victoria'],
			...
			var newData = {'flightnumber': num, 'date': newdate, 'departcity': dptcity, 
					'departtime': dptime, 'tfhour': tfTime, 'arrivalcity': rtncity, 'arrivaltime': artime, 'price': price};
